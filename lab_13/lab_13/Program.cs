...
class Program
{
    static void Main(string[] args)
    {
        try
        {
            ...
            Serialization.JsonSerializion(new Test(name: "Natasha", mark: 4, time: 42));
            Serialization.JsonSerializion(new Test(name: "Andrey", mark: 5, time: 42));
            Serialization.JsonDeserializion();

            Console.WriteLine("Xml\n\n");
            Serialization.XmlSerializion(new TestXml(name: "Egor", age: 32));
            Serialization.XmlDeserializion();


            Console.WriteLine("Binary\n\n");
            Serialization.BinarySerializion(new Test(name: "Andrey", mark: 4, time: 42));
            Serialization.BinaryDeserializion();

            List<Test> tests = new List<Test>();
            tests.Add(new Test(name: "Misha", mark: 4, time: 42));
            tests.Add(new Test(name: "sasha", mark: 4, time: 42));
            tests.Add(new Test(name: "tolua", mark: 4, time: 42));

            List<TestXml> tests1 = new List<TestXml>();
            tests1.Add(new TestXml(name: "nat", age: 32));
            tests1.Add(new TestXml(name: "stacy", age: 32));
            tests1.Add(new TestXml(name: "kira", age: 32));
